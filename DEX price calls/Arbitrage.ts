...
export function getBestCrossedMarket(crossedMarkets: Array<EthMarket>[], tokenAddress: string, baseTokenAddress: string): CrossedMarketDetails | undefined {
  ...
  for (const crossedMarket of crossedMarkets) {
    ...
    for (const size of TEST_VOLUMES) {
      const tokensOutFromBuyingSize = buyFromMarket.getTokensOut(baseTokenAddress, tokenAddress, size);
      const proceedsFromSellingTokens = sellToMarket.getTokensOut(tokenAddress, baseTokenAddress, tokensOutFromBuyingSize)
      ...
      if (bestCrossedMarket !== undefined && profit.lt(bestCrossedMarket.profit)) {
        ...
        const tryTokensOutFromBuyingSize = buyFromMarket.getTokensOut(baseTokenAddress, tokenAddress, trySize);
        const tryProceedsFromSellingTokens = sellToMarket.getTokensOut(tokenAddress, baseTokenAddress, tryTokensOutFromBuyingSize)
...
export class Arbitrage {
  ...
  constructor(executorWallet: Wallet, flashbotsProvider: FlashbotsBundleProvider, bundleExecutorContract: Contract, baseTokenAddress: string) {
  ...
  static printCrossedMarket(crossedMarket: CrossedMarketDetails): Object {
    const crossedMarketObj = {
      "Profit": bigNumberToDecimal(crossedMarket.profit),
      "Volume": bigNumberToDecimal(crossedMarket.volume),
      "BuyFromMarket": {
        "Protocol": crossedMarket.buyFromMarket.protocol,
        "MarketAddress": crossedMarket.buyFromMarket.marketAddress,
        "BuyTokens": crossedMarket.buyFromMarket.tokens
      },
      "SellToMarket": {
        "Protocol": crossedMarket.sellToMarket.protocol,
        "MarketAddress": crossedMarket.sellToMarket.marketAddress,
        "SellTokens": crossedMarket.sellToMarket.tokens
      }
    }
    return crossedMarketObj
  }
  ...
  async evaluateMarkets(marketsByToken: MarketsByToken): Promise<Array<CrossedMarketDetails>> {
    ...
    for (const tokenAddress in marketsByToken) {
      ...
      const pricedMarkets = _.map(markets, (ethMarket: EthMarket) => {
        return {
          ethMarket: ethMarket,
          buyTokenPrice: ethMarket.getTokensIn(tokenAddress, this.baseTokenAddress, ETHER.div(100)),
          sellTokenPrice: ethMarket.getTokensOut(this.baseTokenAddress, tokenAddress, ETHER.div(100)),
        }
      });
      ...
      const bestCrossedMarket = getBestCrossedMarket(crossedMarkets, tokenAddress, this.baseTokenAddress);
  ...
  async takeCrossedMarkets(bestCrossedMarkets: CrossedMarketDetails[], blockNumber: number, minerRewardPercentage: number): Promise<void> {
    for (const bestCrossedMarket of bestCrossedMarkets) {
      ...
      const buyCalls = await bestCrossedMarket.buyFromMarket.sellTokensToNextMarket(this.baseTokenAddress, bestCrossedMarket.volume, bestCrossedMarket.sellToMarket);
      const inter = bestCrossedMarket.buyFromMarket.getTokensOut(this.baseTokenAddress, bestCrossedMarket.tokenAddress, bestCrossedMarket.volume)
