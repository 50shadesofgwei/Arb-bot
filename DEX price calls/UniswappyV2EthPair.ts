...
export class UniswappyV2EthPair extends EthMarket {
  private uniswapInterface: Contract;
  private _tokenBalances: TokenBalances;

  constructor(marketAddress: string, tokens: Array<string>, protocol: string, baseTokenAddress: string) {
    super(marketAddress, tokens, protocol, baseTokenAddress);
    this._tokenBalances = _.zipObject(tokens,[BigNumber.from(0), BigNumber.from(0)])
    this.baseTokenAddress = baseTokenAddress;
    this.uniswapInterface = new Contract(this.baseTokenAddress, UNISWAP_PAIR_ABI);
  }
  ...
  static async getUniswappyMarkets(provider: providers.JsonRpcProvider, factoryAddress: string, baseTokenAddress: string): Promise<Array<UniswappyV2EthPair>> {
    ...
    for (let i = 0; i < BATCH_COUNT_LIMIT * UNISWAP_BATCH_SIZE; i += UNISWAP_BATCH_SIZE) {
      ...
      for (let i = 0; i < pairs.length; i++) {
        ...
        if (pair[0] === baseTokenAddress) {
          tokenAddress = pair[1]
        } else if (pair[1] === baseTokenAddress) {
          tokenAddress = pair[0]
        } else {
          continue;
        }
        if (!blacklistTokens.includes(tokenAddress)) {
          const uniswappyV2EthPair = new UniswappyV2EthPair(marketAddress, [pair[0], pair[1]], "", baseTokenAddress);
  ...
  static async getUniswapMarketsByToken(provider: providers.JsonRpcProvider, factoryAddresses: Array<string>, baseTokenAddress: string): Promise<GroupedMarkets> {
    const allPairs = await Promise.all(
      _.map(factoryAddresses, factoryAddress => UniswappyV2EthPair.getUniswappyMarkets(provider, factoryAddress, baseTokenAddress))
    )

    const marketsByTokenAll = _.chain(allPairs)
      .flatten()
      .groupBy(pair => pair.tokens[0] === baseTokenAddress ? pair.tokens[1] : pair.tokens[0])
      .value()
    ...
    const marketsByToken = _.chain(allMarketPairs)
      .filter(pair => (pair.getBalance(baseTokenAddress).gt(ETHER)))
      .groupBy(pair => pair.tokens[0] === baseTokenAddress ? pair.tokens[1] : pair.tokens[0])
      .value()
  ...
  async sellTokens(tokenIn: string, amountIn: BigNumber, recipient: string): Promise<string> {
    ...
    const populatedTransaction = await this.uniswapInterface.populateTransaction.swap(amount0Out, amount1Out, recipient, []);
